/*
  SRT Ticket Tools – Correctness Patch v6
  ใช้กับ index.html เดิม โดยวางไฟล์นี้ไว้โฟลเดอร์เดียวกัน
  และเพิ่ม <script src="booking-fix-v6.js"></script> หลัง </script> เดิม ก่อน </body>
*/
(() => {
  'use strict';
  ...
  const SPECIAL_997998_TRAIN_IDS = new Set(['997/998']);
  const SPECIAL_997998_BOUNDARIES = {
    chachoengsaoKm: 61,
    chonburiKm: 108
  };
  ...
  function classify997998Route(originKm, destinationKm) {
    if (!Number.isFinite(originKm) || !Number.isFinite(destinationKm)) return null;

    const minKm = Math.min(originKm, destinationKm);
    const maxKm = Math.max(originKm, destinationKm);
    const { chachoengsaoKm, chonburiKm } = SPECIAL_997998_BOUNDARIES;

    // ช่วงกรุงเทพ/สถานีฝั่งกรุงเทพ ถึงชุมทางฉะเชิงเทรา:
    // D-Ticket แสดงเงื่อนไข “ห้ามขายก่อนรถออก 48 ชั่วโมง”
    if (maxKm <= chachoengsaoKm) return 'special-48h';

    // เดินทางข้ามจากฝั่งกรุงเทพ (ก่อนถึงฉะเชิงเทรา)
    // ไปตั้งแต่ชลบุรีขึ้นไป ใช้หลักเกณฑ์ระยะทาง 1 / 30 / 90 วัน
    if (minKm < chachoengsaoKm && maxKm >= chonburiKm) return 'standard-distance';

    // ช่วงฉะเชิงเทราไปทางชลบุรี/พัทยา/สัตหีบ
    // และช่วงภายในฝั่งชลบุรี–พัทยา–สัตหีบ:
    // จากการทดสอบใน D-Ticket เปิดขายล่วงหน้า 1 วัน
    return 'special-1day';
  }
  ...
  function renderSpecial48Hour(travelDate) {
    ...
    let statusHtml = '';
    if (detail.saleStart) {
      statusHtml = now < detail.saleStart
        ? '<div class="patch-status waiting">⏳ ยังไม่เข้าสู่ช่วงจำหน่าย 48 ชั่วโมง</div>'
        : '<div class="patch-status open">✅ เข้าสู่ช่วงจำหน่ายแล้ว — โปรดลองทำรายการใน D-Ticket หรือติดต่อสถานี</div>';
    } else {
      statusHtml = `
    ...
    result.innerHTML = `
      <div class="east">
        ...
        <h3>ขบวน ${detail.trainNo} ช่วงอัตราพิเศษ 48 ชั่วโมง</h3>
        <p><b>ใช้กับการเดินทางในช่วงกรุงเทพ–ชุมทางฉะเชิงเทรา</b></p>
        ...
        ${weekendWarning}
        <div class="patch-small">
          เงื่อนไขนี้อ้างอิงจากข้อความที่ปรากฏใน D-Ticket สำหรับช่วงค่าโดยสารดังกล่าว
          หากขึ้นจากสถานีระหว่างทาง โปรดตรวจเวลาขบวนออกจากสถานีนั้นอีกครั้ง
        </div>
  ...
  function renderSpecialOneDay(travelDate) {
    const result = $('result');
    if (!result) return;
    ...
    const statusHtml = now < opensAt
      ? '<div class="patch-status waiting">⏳ ยังไม่เปิดขาย — เปิดล่วงหน้า 1 วัน</div>'
      : '<div class="patch-status open">✅ เปิดขายแล้ว — ทั้งนี้ขึ้นอยู่กับที่นั่งว่างและเงื่อนไขใน D-Ticket</div>';

    result.innerHTML = `
      <div class="booking gray">
        <div class="booking-inner">
          ${statusHtml}
          <div class="pill">ขบวน 997/998 • ช่วงค่าโดยสารพิเศษภายในสายตะวันออก</div>
          <div class="days gray">ซื้อล่วงหน้า 1 วัน</div>
          <div class="datebox">
            <div class="label">วันที่เริ่มซื้อตั๋วได้คือ</div>
            <div class="date">${formatDate(bookingDate)}</div>
            <div style="font-size:12px;color:#64748b;font-weight:800">
              สำหรับเดินทางวันที่ ${formatDate(travelDate)}
              จาก ${escapeHtml(origin)} ไป ${escapeHtml(destination)}
            </div>
            <div class="time">⏰ ระบบคำนวณวันเริ่มขายที่เวลา 08.30 น. โปรดตรวจสอบ D-Ticket อีกครั้ง</div>
            <div class="patch-small">
              ใช้กับช่วงชุมทางฉะเชิงเทราไปทางชลบุรี/พัทยา/สัตหีบ
              และการเดินทางภายในฝั่งชลบุรี–พัทยา–สัตหีบ
              ตามผลทดสอบในระบบ D-Ticket ของผู้จัดทำ
            </div>
            <a class="patch-action" href="${OFFICIAL_DTICKET_URL}" target="_blank" rel="noopener noreferrer">
              เปิดเว็บไซต์ D-Ticket
            </a>
  ...
  function renderBookingResult({ percentage, days, bookingDate, travelDate, routeNote = '' }) {
    const result = $('result');
    if (!result) return;
      ...
      <div class="booking ${color}">
        <div class="booking-inner">
          ...
          <div class="datebox">
            ...
            ${routeNote ? `<div class="patch-small">${escapeHtml(routeNote)}</div>` : ''}
            <a class="patch-action" href="${OFFICIAL_DTICKET_URL}" target="_blank" rel="noopener noreferrer">
              เปิดเว็บไซต์ D-Ticket
            </a>
          </div>
  ...
  function calculateBookingPatched() {
    ...
    // ขบวน 997/998 มีเงื่อนไขต่างกันตามช่วงสถานี
    if (mode === 'station' && selectedTrain && SPECIAL_997998_TRAIN_IDS.has(selectedTrain.id)) {
      ...
      const originKm = getKm($('passOrigin')?.value);
      const destinationKm = getKm($('passDest')?.value);
      const routeRule = classify997998Route(originKm, destinationKm);

      if (routeRule === 'special-48h') {
        renderSpecial48Hour(travelDate);
        return;
      }

      if (routeRule === 'special-1day') {
        renderSpecialOneDay(travelDate);
        return;
      }

      if (routeRule === 'standard-distance') {
        ...
        renderBookingResult({
          ...
          bookingDate,
          travelDate,
          routeNote: 'ขบวน 997/998 จากฝั่งกรุงเทพไปตั้งแต่ชลบุรีขึ้นไป ใช้หลักเกณฑ์ระยะทาง 1 / 30 / 90 วัน'
        });
        return;
      }

      renderBookingError('ยังไม่สามารถระบุเงื่อนไขการจำหน่ายของช่วงสถานีที่เลือกได้ โปรดตรวจสอบกับ D-Ticket');
      return;
    }
  ...
  function updateSpecialTrainLabels() {
    ...
    if (option) {
      option.textContent = '997/998 พิเศษโดยสาร (เงื่อนไขแตกต่างตามช่วงสถานี)';
    }

    document.querySelectorAll('.notice li').forEach((item) => {
      if (item.textContent.includes('997/998')) {
        item.textContent = 'ขบวน 997/998 มีเงื่อนไขตามช่วงสถานี: กรุงเทพ–ฉะเชิงเทราเปิดขายภายใน 48 ชั่วโมงก่อนรถออก; ฝั่งกรุงเทพไปตั้งแต่ชลบุรีขึ้นไปใช้เกณฑ์ 1 / 30 / 90 วัน; ช่วงฉะเชิงเทราไปทางชลบุรี/พัทยา/สัตหีบและช่วงภายในฝั่งตะวันออกเปิดขายล่วงหน้า 1 วัน ตามผลทดสอบ D-Ticket';
      }
    });
  }
  ...
  function professionalizeNotices() {
    ...
    if (info) {
      info.innerHTML = `
        ...
        <span class="patch-version">PATCH v6</span>
      `;
    }
  ...
  function init() {
    ...
    console.info('SRT Ticket Tools: Correctness Patch v6 loaded');
  }
